...
let matrix3 = new Uint8ClampedArray(
  ROWS * INST_SIDE * COLS * INST_SIDE * LOOP_DURATION * Px * Px * 4
); // scaled matrix, 4 is due to Uint8ClampedArray bytes
...
async function createMatrix(path) {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      let [onsets, velocities, timeshifts] = vae.generatePattern(
        c_norm,
        r_norm,
        0
      );
      for (let i = 0; i < NUM_DRUM_CLASSES; i++) {
        ...
        matrix.set(
          onsets[i],
          ((COLS * r + c) * NUM_DRUM_CLASSES + i) * LOOP_DURATION
        );
      }
    }
  }
  ...
  function scaleMatrix(t, r, c, pos, val) {
    ...
    for (let y = 0; y < Px; y++) {
      for (let x = 0; x < Px; x++) {
        ...
        if (val >= 0 && pos % INST_SIDE == 0)
          matrix3.set([val * 255, 0, 0, 255], idx);
        else if (val >= 0 && pos % INST_SIDE == 1)
          matrix3.set([0, val * 255, 0, 255], idx);
        else if (val >= 0 && pos % INST_SIDE == 2)
          matrix3.set([0, 0, val * 255, 255], idx);
      }
    }
  }
  ...
  for (let t = 0; t < LOOP_DURATION; t++) {
    for (let r = 0; r < ROWS * INST_SIDE; r++) {
      for (let c = 0; c < COLS * INST_SIDE; c++) {
        pos =
          r * COLS * INST_SIDE + c + t * ROWS * INST_SIDE * COLS * INST_SIDE;
        val = matrix2[pos];
  ...
  function generateRGB() {
    ...
    for (let t = 0; t < LOOP_DURATION; t++) {
      matrix4 = matrix3.slice(
        width * height * 4 * t,
        width * height * 4 * (t + 1)
      );

      visualizationDict[t] = {
        r: Array.from(matrix4.filter((_, index) => index % 4 == 0)),
        g: Array.from(matrix4.filter((_, index) => index % 4 == 1)),
        b: Array.from(matrix4.filter((_, index) => index % 4 == 2)),
        a: Array.from(matrix4.filter((_, index) => index % 4 == 3)),
      };
    }
  }
}
