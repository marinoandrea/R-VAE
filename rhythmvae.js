...
function isValidMIDIFile(midiFile) {
  if (midiFile.header.tempos.length > 1) {
    utils.error(
      "not compatible with midi files containing multiple tempo changes"
    );
    return false;
  }
  return true;
...
function processPianoroll(midiFile) {
  ...
  for (var i = 0; i < onsets.length; i++) {
    if (getNumOfDrumOnsets(onsets[i]) > MIN_ONSETS_THRESHOLD) {
      train_data_onsets.push(
        tf.tensor2d(onsets[i], [NUM_DRUM_CLASSES, LOOP_DURATION])
      );
      train_data_velocities.push(
        tf.tensor2d(velocities[i], [NUM_DRUM_CLASSES, LOOP_DURATION])
      );
      train_data_timeshifts.push(
        tf.tensor2d(timeshifts[i], [NUM_DRUM_CLASSES, LOOP_DURATION])
      );
    }
  }
}
...
Max.addHandler("train", () => {
  if (vae.isTraining()) {
    utils.error_status(
      "Failed to start training. There is already an ongoing training process."
    );
    return;
  }
    ...
    for (var i = 0; i < NUM_DRUM_CLASSES; i++) {
      ...
      for (var j = 0; j < LOOP_DURATION; j++) {
        ...
        if (onsets[i][j] > threshold) {
          ...
          sequenceTS.push(
            Math.floor(utils.scale(timeshifts[i][j], -1, 1, 0, 127))
          ); // -1 - 1 -> 0 - 127
        } else {
          sequence.push(0);
          sequenceTS.push(64);
...
async function createMatrix(path) {
  ...
  let matrix2 = new Float32Array(
    ROWS * COLS * LOOP_DURATION * NUM_DRUM_CLASSES
  );
